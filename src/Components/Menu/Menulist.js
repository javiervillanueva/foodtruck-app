...
class Menulist extends React.Component {
  componentDidMount() {
    axios.get("/api/get-menu-items-id").then(response => {
  ...
  render() {
          ...
          <div className="topBox">
            ...
            <div className="textBoxes">
              <div className="foodBox">
                <h1 className="foodName">{vendorMenu.title}</h1>
              </div>
              <div className="moneyBox">
                {" "}
                <h1 className="cost">${vendorMenu.price}</h1>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="menuWrapper">
        <div className="menuCard">{MenuCard}</div>

        <Fab className="addMenuItem" href="/vendor/menuadd">
          <AddIcon />
        </Fab>
      </div>
    );
